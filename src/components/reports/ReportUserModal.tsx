...
export default function ReportUserModal({ isOpen, onClose, reportedUserId, reportedUserName, conversationId }: ReportUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/reports/user", {
        ...
        body: JSON.stringify({ reason, description, reportedUserId, conversationId }),
      });
          ...
          <>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">Report <span className="text-slate-700 font-medium">{reportedUserName}</span></p>
              </div>
            </div>
