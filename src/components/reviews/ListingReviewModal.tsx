...
export default function ListingReviewModal({ isOpen, onClose, listingId, conversationId, existingReview }: ListingReviewModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = existingReview ? `/api/listing-reviews/${existingReview.id}` : "/api/listing-reviews";
      const method = existingReview ? "PATCH" : "POST";
      const bodyPayload = existingReview
        ? { rating, comment }
        : { rating, comment, listingId, conversationId };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 w-[95vw] sm:w-[500px] max-w-full max-h-[90vh] overflow-y-auto shadow-xl relative">
        ...
        <h2 className="text-2xl font-bold text-white mb-6">
          {existingReview ? "Edit Listing Review" : "Leave a Listing Review"}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="How was your experience with this advertising space?"
